...
function createNodeBuild() {
  return (res, rej) => {
    let count = 0;
    let bat = exec('NODE_ENV=production babel ./src/lib --out-dir ./build/lib --copy-files', { stdio: [0, 1, 2] }, (error, stdout, stderr) => {
      if (error) {
        rej(error);
        return;
      }
...
rimraf('build', () => {
  ...
  fs.mkdirSync('build');
  // create the lib folder for NODE+UMD bundles
  fs.mkdirSync(join('build', 'lib'));
